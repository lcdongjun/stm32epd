...
void invertPixelColor(uint8_t *image, uint16_t x, uint16_t y) 
{
    ...
    // 计算像素所在字节的位置
    uint32_t byteIndex = (y * (Paint.WidthByte)) + (x / 8);
    // 计算像素在字节中的位位置
...
void Paint_Show_Line(u16 x_start, u16 y_start, u16 x_end, u16 y_end, u16 color, u8 invPix)
{
    ...
    while (1) {
			if(invPix)
			{
			invertPixelColor(Paint.Image, x_start, y_start);
			}
		 else
		 {
...
void Paint_Show_Rectangle(u16 x_start, u16 y_start, u16 x_end, u16 y_end,u8 thickness, u16 color, u8 filled, u8 invPix)
{
    if (filled) {
        for (u16 y = y_start; y <= y_end; y++) {
            Paint_Show_Line(x_start, y, x_end, y, color, invPix);
        }
    } else {
			for(;thickness>=1;thickness--)
			{
				Paint_Show_Line(x_start, y_start, x_end, y_start, color ,invPix);  // Top edge
        Paint_Show_Line(x_start, y_end, x_end, y_end, color ,invPix);      // Bottom edge
        Paint_Show_Line(x_start, y_start, x_start, y_end, color ,invPix);  // Left edge
        Paint_Show_Line(x_end, y_start, x_end, y_end, color ,invPix);      // Right edge
				x_start++;
				y_start++;
				x_end--;
				y_end--;
...
void Paint_Show_Circle(u16 x_center, u16 y_center, u16 radius, u8 thickness, u16 color, u8 filled, u8 invPix)
{
    ...
    if (thickness == 1) {
        ...
        while (x >= y) {
            if (filled) {
                // 绘制填充圆，画四条水平和垂直的线段
                Paint_Show_Line(x_center - x, y_center + y, x_center + x, y_center + y, color, invPix);
                Paint_Show_Line(x_center - y, y_center + x, x_center + y, y_center + x, color, invPix);
                Paint_Show_Line(x_center - x, y_center - y, x_center + x, y_center - y, color, invPix);
                Paint_Show_Line(x_center - y, y_center - x, x_center + y, y_center - x, color, invPix);
            } else {
                // 绘制空心圆
...
void Paint_Show_Arc(u16 x_center, u16 y_center, u16 radius, u16 start_angle, u16 end_angle, u8 thickness, u16 color, u8 filled, u8 invPix)
{
    ...
    for (float angle = radian_start + angle_step; angle <= radian_end; angle += angle_step) {
        ...
        // 使用画线函数连接相邻的点，形成弧线
        Paint_Show_Line(x_prev, y_prev, x, y, color, invPix);
    ...
    if (filled) {
        for (float angle = radian_start + angle_step; angle <= radian_end; angle += angle_step) {
            u16 x = x_center + (int)(radius * arm_cos_f32(angle));
            u16 y = y_center + (int)(radius * arm_sin_f32(angle));
            Paint_Show_Line(x_center, y_center, x, y, color, invPix);
        }
    }
...
void Paint_Show_RoundRect(u16 x_start, u16 y_start, u16 x_end, u16 y_end, u16 radius, u8 thickness, u16 color, u8 filled, u8 invPix)
{
    ...
    if (filled) {
				if(invPix)
				{
					Paint_Show_Rectangle(x_start,y_start+radius+1, x_end,y_end-radius-1, thickness, color,1,invPix);
					Paint_Show_Rectangle(x_start+radius,y_start,x_end-radius,y_start+radius,thickness,  color,1,invPix);
					Paint_Show_Rectangle(x_start+radius,y_end-radius, x_end-radius,y_end, thickness, color,1,invPix);

					// 绘制圆角部分的四个圆弧
					Paint_Show_Arc(x_start + radius, y_start + radius, radius, 180, 270, 1,color,1,invPix); // 左上
					Paint_Show_Arc(x_end - radius, y_start + radius, radius, 270, 360, 1,color,1,invPix);   // 右上
					Paint_Show_Arc(x_start + radius-1, y_end - radius-1, radius, 90, 180, 1,color,1,invPix);// 左下
					Paint_Show_Arc(x_end - radius, y_end - radius, radius, 0, 90, 1,color,1,invPix);       // 右下
				}
				else
				{
					Paint_Show_Rectangle(x_start,y_start+radius, x_end,y_end-radius, thickness, color,1, invPix);
					Paint_Show_Rectangle(x_start+radius,y_start,x_end-radius,y_start+radius,thickness,  color,1, invPix);
					Paint_Show_Rectangle(x_start+radius,y_end-radius, x_end-radius,y_end, thickness, color,1, invPix);
					...
					Paint_Show_Arc(x_start + radius, y_start + radius, radius, 180, 270, 1,color,1, invPix); // 左上
					Paint_Show_Arc(x_end - radius, y_start + radius, radius, 270, 360, 1,color,1, invPix);   // 右上
					Paint_Show_Arc(x_start + radius, y_end - radius, radius, 90, 180, 1,color,1, invPix);    // 左下
        Paint_Show_Arc(x_end - radius, y_end - radius, radius, 0, 90, 1,color,1, invPix);       // 右下
				}
    } else {
        // 绘制空心矩形的边界（去除圆角部分）
				Paint_Show_Rectangle(x_start + radius, y_start, x_end - radius, y_start+thickness,thickness,color,1, invPix);
				Paint_Show_Rectangle(x_start + radius, y_end-thickness, x_end - radius, y_end,thickness,color,1, invPix);	
				Paint_Show_Rectangle(x_start, y_start+radius, x_start+thickness, y_end- radius,thickness,color,1, invPix);
				Paint_Show_Rectangle(x_end, y_start+radius, x_end-thickness, y_end- radius,thickness,color,1, invPix);

        // 绘制圆角部分的四个圆弧
        Paint_Show_Arc(x_start + radius, y_start + radius, radius, 180, 270, thickness,color,0, invPix); // 左上
        Paint_Show_Arc(x_end - radius, y_start + radius, radius, 270, 360, thickness,color,0, invPix);   // 右上
        Paint_Show_Arc(x_start + radius, y_end - radius, radius, 90, 180, thickness,color,0, invPix);    // 左下
        Paint_Show_Arc(x_end - radius, y_end - radius, radius, 0, 90, thickness,color,0, invPix);       // 右下
    }
}
...
void Paint_Show_Triangle(u16 x0, u16 y0, u16 x1, u16 y1, u16 x2, u16 y2, u16 color, u8 filled, u8 invPix)
{
    if (filled) {
        ...
        // 绘制空心三角形的三条边
        Paint_Show_Line(x0, y0, x1, y1, color, invPix);
        Paint_Show_Line(x1, y1, x2, y2, color, invPix);
        Paint_Show_Line(x2, y2, x0, y0, color, invPix);
    }
}
...
{
    int16_t Num_Bit = 0, Str_Bit = 0;
    uint8_t *Str_Array, *Num_Array;

		Str_Array = mymalloc(SRAMIN,ARRAY_LEN);
		Num_Array = mymalloc(SRAMIN,ARRAY_LEN);
		memset(Str_Array, 0, ARRAY_LEN); // 将申请的内存清零
		memset(Num_Array, 0, ARRAY_LEN); // 将申请的内存清零
    uint8_t *pStr = Str_Array;
    int is_negative = 0;
    ...
    Paint_Show_Str(x, y, pStr, size, mode, cover);
		myfree(SRAMIN,Str_Array);
		myfree(SRAMIN,Num_Array);
}
